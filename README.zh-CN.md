...
| 💾 保存 BHV | `Ctrl+S` | 保存并下载修改后的 `.bhv` 文件 |
| ➕ 新建状态 | — | 在列表末尾添加一个新的空状态 |
| ↩ 撤销 | `Ctrl+Z` | 撤销上一步编辑操作 |
| ↪ 重做 | `Ctrl+Y` 或 `Ctrl+Shift+Z` | 重做被撤销的操作 |
| 🌐 中/EN | — | 切换中英文界面 |
...
| `Ctrl+D` | 复制当前选中的状态 |
| `Ctrl+F` | 聚焦搜索框 |
| `Delete` | 删除所有选中的状态（需确认）|
| `Escape` | 关闭任何打开的弹窗/对话框 |
...
| 标签 | 面板 | 说明 |
|-----|------|------|
| 🕸 状态机编辑 | **画布面板** | 可视化状态机图编辑器（默认）|
| 📊 StructB 编辑 | **StructB 面板** | 编辑 StructB 二进制配置条目 |
| 🔢 StructC 编辑 (Hex) | **StructC 面板** | 以十六进制编辑 StructC 数据 |
| 📝 Strings 编辑 | **Strings 面板** | 编辑字符串列表 |
| 📋 State Action | **StateAction 面板** | 查看和编辑当前选中状态的属性 |
...
│ [100%] [+] [−] [⊞ 适应] [节点:0] [2跳] [力导] [网格] [树状] [3D] [0,0] │
│                                                      │
│         ┌─────┐     ┌─────┐                          │
│         │ S0  │────→│ S1  │                          │
│         │ idle│     │atk1 │                          │
...
| 按钮 | 布局 | 说明 |
|------|------|------|
| **力导** | 力导向布局 | 自动排列节点，相连的状态聚在一起。最适合查看过渡关系结构。|
| **网格** | 网格布局 | 将所有状态排列成规则的网格。最适合一目了然地查看所有状态。|
| **树状** | 树形布局 | 从根节点开始按 BFS 层级排列。最适合查看过渡层次结构。|
| **3D** | 3D 视图切换 | 在 2D 画布和 3D WebGL 视图之间切换。节点、连线在 3D 空间中渲染。再次点击返回 2D。|

**过滤与搜索：**

| 控制 | 说明 |
|------|------|
| Anim1 ID 过滤 | 高亮显示匹配动画 ID (Unk04) 的状态 |
| Anim2 ID 过滤 | 次要动画过滤 |
| 搜索框 | 按名称或索引查找状态 — 下方显示匹配结果 |
| 焦点层级 (N跳) | 限制从选中状态出发显示的过渡跳数 |
| 小地图 | 右下角整个状态图的缩略概览；点击可导航 |
...
**StructB 面板** — 编辑 StructB 条目（二进制配置数组）。每行代表一个 StructB 条目，字段包括：Unk04, Unk08, Unk0C, Unk10。

**StructC 面板** — 编辑 StructC 条目（十六进制整数数组）。直接点击单元格即可编辑十六进制值。

**Strings 面板** — 编辑字符串列表。可添加、删除或修改字符串条目。常用字符串包括动画名称、事件名称等。

**StateAction 面板** — 查看和编辑当前选中的状态的详细属性。显示选中 State 的所有字段，包括其 Transitions 和 Conditions。
