...
| 类型 | 文件名 | 说明 |
|------|--------|------|
| `basenormal.bhv` | `basenormal.bhv` | 基础敌人/士兵行为 |
| `weapon.bhv` | `weapon.bhv` | 武器行为 |
| `w.bhv` | `w.bhv` | 通用/世界行为 |
...
- **状态列表** — 查看所有状态及其索引
- **过渡编辑器** — 添加/编辑/删除状态之间的过渡
- **条件编辑器** — 设置每个过渡的触发条件
- **二进制预览** — 查看 StructB/StructC/StructD 的原始十六进制数据
- **JSON 导入/导出** — 用于批量编辑或备份
- **调试覆盖** — 加载调试 JSON 查看状态名称
